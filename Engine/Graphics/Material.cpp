...
{
	std::fstream file("/Users/hernan/Documents/learn/Engine/Graphics/Materials/" + filename, std::ios_base::in);

	if (!file)
	{
		std::cout << "Error loading material file" << std::endl;
		Utility::AddMessage("Error loading material file " + filename);
		return false;
	}
	...
	while (!file.eof())
	{
		...
		if (!line.empty() && line[0] != '#')
		{
			...
			{
				materials.back().m_ambientMap.Load("/Users/hernan/Documents/learn/Engine/Graphics/Textures/" + subStrings[1]);
			...
			{
				materials.back().m_diffuseMap.Load("/Users/hernan/Documents/learn/Engine/Graphics/Textures/" + subStrings[1]);
				materials.back().m_isTextured = true;
				continue;
			}
			...
			{
				materials.back().m_specularMap.Load("/Users/hernan/Documents/learn/Engine/Graphics/Textures/" + subStrings[1]);
				materials.back().m_isTextured = true;
				continue;
			}
			...
			{
				materials.back().m_normalMap.Load("/Users/hernan/Documents/learn/Engine/Graphics/Textures/" + subStrings[1]);
				materials.back().m_isTextured = true;
				continue;
			}
		}
